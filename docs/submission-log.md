...
| 3 | _pending upload_ | v0.4-ismcts-core (planned) | _pending_ | SO-ISMCTS (`submissions/ismcts_main.py` + bundled `env/` + `search/`) | Phase 0 placeholder (unchanged) | iterations=500, c=√2, **filler determinization** (opponent deck unknown on ladder → Basic-filler, weaker than the EXP-003 mirror setup); per-decision heuristic fallback | _pending_ | _pending_ | Beats the heuristic (527.9) on the ladder. Caveat: ladder ISMCTS uses filler determinization, a lower bound vs the mirror-deck EXP-003 (78% local). Timing risk: 500 iters chosen for a ~2× wall-clock margin against the 10-min/match limit on the 2-vCPU worker; watch the Validation Episode for timeouts. | _pending_ |
